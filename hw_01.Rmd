...
number_double = x_vector != y_vector
print(sum(number_double))
```
...
was keeping track of parentheses to ensure I would not run an error. To compare
inputs, I used the strsplit() function to break apart words/numbers into
individual components. I then unlisted them to turn each word into a vector of
its letters.
...
message. I tested all of the steps on their own outside of the function and
received the correct results, yet when the statement was in the function, it
continued to output -1.
